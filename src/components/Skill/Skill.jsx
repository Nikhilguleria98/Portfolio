...
const Skills = () => {
  const skills = [
    ...
    { name: "Tailwind CSS", icon: <FaCss3Alt size={50} />, color: "bg-gradient-to-r from-teal-500 to-teal-900", hover: "hover:shadow-teal-900", level: "Advanced" },
      ...
      <div className="max-w-6xl mx-auto px-6 text-center  text-white">
        ...
        >
          {skills.map((skill, index) => (
            <motion.div
              key={index}
              className={`p-6 rounded-xl shadow-lg ${skill.color} ${skill.hover} transition-all transform hover:scale-105 hover:shadow-lg duration-300 cursor-pointer`}
            ...
            >
              ...
              <p className="text-white opacity-80">{skill.level}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
